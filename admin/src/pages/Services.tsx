    ...
    <div>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ width: '40px', height: '40px', borderRadius: '8px', backgroundColor: 'var(--color-bg-secondary)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyItems: 'center' }}>
                         {service.image_url ? (
                           // Just a placeholder since it could be local assets in app. Real URL would use storage public url.
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingItem ? 'Modifier Prestation' : 'Nouvelle Prestation'}>
        <form onSubmit={handleSubmit}>
          ...
          <Input label="URL de l'image (Nom du fichier local ou lien Web)" value={formData.image_url} onChange={(e:any) => setFormData({...formData, image_url: e.target.value})} />
